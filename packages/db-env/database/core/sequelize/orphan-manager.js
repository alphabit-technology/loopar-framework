...
const IMMUNE_COLUMNS = new Set(['id', 'name', '__document_status__']);
...
export class OrphanManager {
  ...
  async releaseOrphanColumn(orm, document, columnName) {
    ...
    if (dialect.includes('mysql')) {
      ...
      await orm.sequelize.query(
        `ALTER TABLE ${tableName} MODIFY COLUMN ${col} ${rawType} NULL`,
        { type: QueryTypes.RAW, transaction: orm.transaction }
      );
  ...
  async dropOrphanColumn(orm, document, columnName) {
    ...
    if (orm.dialect.includes('sqlite')) {
      await this.dropColumnSQLiteRecreate(orm, document, tableName, columnName);
    } else {
      await this.dropColumnIndexes(orm, tableName, columnName);
      await orm.sequelize.query(
        `ALTER TABLE ${tableName} DROP COLUMN ${orm.escapeId(columnName)}`,
        { type: QueryTypes.RAW, transaction: orm.transaction }
      );
    }
  ...
  async dropColumnSQLiteRecreate(orm, document, tableName, columnName) {
    ...
    try {
      await orm.sequelize.query(`SAVEPOINT drop_col`, { type: QueryTypes.RAW });

      await orm.sequelize.query(createSQL, { type: QueryTypes.RAW });
      ...
      await orm.sequelize.query(`DROP TABLE ${tableName}`, { type: QueryTypes.RAW });

      await orm.sequelize.query(
        `ALTER TABLE ${tmpName} RENAME TO \`${literalName}\``,
        { type: QueryTypes.RAW }
      );

      await orm.sequelize.query(`RELEASE SAVEPOINT drop_col`, { type: QueryTypes.RAW });
      ...
      try {
        await orm.sequelize.query(`ROLLBACK TO SAVEPOINT drop_col`, { type: QueryTypes.RAW });
        await orm.sequelize.query(`DROP TABLE IF EXISTS ${tmpName}`, { type: QueryTypes.RAW });
  ...
  async reconcile(orm, tableName, orphans) {
    ...
    for (const orphan of orphans) {
      if (dialect.includes('mysql')) {
        ...
        // SQLite: only drop indexes here; NOT NULL is covered by insertRow padding
        await this.dropColumnIndexes(orm, tableName, orphan.name);
        console.log(`[OrphanManager] SQLite "${orphan.name}": indexes dropped (NOT NULL via insert padding)`);
      }
    }
  }
  ...
  async dropSQLiteColumnIndexes(orm, tableName, columnName) {
    ...
    const indexes = await orm.sequelize.query(
      `SELECT name, sql FROM sqlite_master
       WHERE type = 'index'
         AND tbl_name = ?
         AND sql IS NOT NULL`,
      { replacements: [literalTable], type: QueryTypes.SELECT }
    );
    ...
    for (const idx of indexes) {
      if (patterns.some(p => p.test(idx.sql || ''))) {
        try {
          await orm.sequelize.query(
            `DROP INDEX IF EXISTS ${orm.escapeId(idx.name)}`,
            { type: QueryTypes.RAW, transaction: orm.transaction }
          );
  ...
  async dropConventionColumnIndexes(orm, tableName, columnName) {
    ...
    if (orm.dialect.includes('mysql')) {
      for (const idx of candidates) {
        try {
          await orm.sequelize.query(
            `ALTER TABLE ${tableName} DROP INDEX ${orm.escapeId(idx)}`,
            { type: QueryTypes.RAW, transaction: orm.transaction }
          );
        } catch (e) { }
      }
      ...
      for (const idx of candidates) {
        try {
          await orm.sequelize.query(
            `DROP INDEX IF EXISTS ${orm.escapeId(idx)}`,
            { type: QueryTypes.RAW, transaction: orm.transaction }
          );
        } catch (e) { }
      }
  ...
  async assertSQLiteDropColumnSupport(orm) {
    ...
    const version = parseFloat(result[0]?.v || '0');
  ...
  sqlTypeToLooparlFormat(rawType) {
    ...
    if (t.includes('DATE')) return 'DATEONLY';
