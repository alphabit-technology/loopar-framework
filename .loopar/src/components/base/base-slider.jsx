...
export default class BaseSlider extends Preassembled {
  ...
  constructor(props) {
    ...
    this.state = {
      ...this.state,
      currentIndex: localStorage.getItem(this.props.data.key) || 0,
      prevIndex: 0,
      focus: false,
      initialized: false,
    };
  ...
  getSlidersRender() {
    ...
    return [
      ...sliders.map((element, index) => {
        ...
        return [
          this.getElement(
            { ...element, element: element.element || "banner" },
            {
              className:
                index == 0 ? "position-absolute hide-time" : "show-time",
              meta: {
                data,
  ...
  getPrevSlideIndex() {
    return (
      (this.state.currentIndex - 1 + this.getSliders().length) %
      this.getSliders().length
    );
  }
  ...
  getChevron(position) {
    return (
      <button
        className={`w3-button w3-display-${position} p-9 transition ${
          this.state.focus ? "" : "ishide"
        }`}
        style={{
          zIndex: "99999999",
          padding: 20,
