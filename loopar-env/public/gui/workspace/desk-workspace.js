...
export default class DeskWorkspace extends BaseWorkspace {
   ...
   render(){
      ...
      return super.render([
         ...
         aside({className: `app-aside app-aside-expand-md app-aside-light ${menu ? 'show' : ''}`, id:"aside"}, [
            div({className: 'aside-content'}, [
               header({className: "aside-header b-block d-md-none"}, [
                  button({className: "btn-account", onClick: () => this.setState({mobileMenuUser: !mobileMenuUser})}, [
               ...
               div({className: 'aside-menu overflow-hidden ps'}, [
                  nav({
                     className: `stacked-menu ${collapseMenu ? "stacked-menu-has-compact stacked-menu-has-hoverable" : "stacked-menu-has-collapsible"}`,
                  }, [
                     DeskSidebar({
         ...
         div({className: `aside-backdrop ${this.state.showBackdrop ? 'show' : ''}`, style: {display: this.state.showBackdrop ? 'block' : 'none'}, onClick: () => {
            this.toggleMenu();
         }})
      ]);
   }
   ...
   bodyToggleMenu(){
      const hasCompactMenu = window.innerWidth > 768;
      document.getElementById("app-root")?.classList[this.state.collapseMenu && hasCompactMenu ? "add" : "remove"]("has-compact-menu");
   }

   collapseMenu(){
      const collapseMenu = !this.state.collapseMenu;
      this.setState({menu: false, showBackdrop: false,  collapseMenu: collapseMenu});
      localStorage.setItem("collapseMenu", collapseMenu);
   }
