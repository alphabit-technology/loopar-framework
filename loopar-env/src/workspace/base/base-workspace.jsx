...
import Theme from "#workspace/base/theme";
...
export default class BaseWorkspace extends Theme {
   ...
   constructor(props) {
      ...
      this.state = {
         ...
         showBackdrop: false,
         menu: false,
         collapseMenu: false,//localStorage.getItem("collapseMenu") === "true",
         progress: 0,
         toProgress: 20,
         screenType: "desktop",
         openDropdowns: [],
   ...
   setDocument(res) {
      ...
      //console.log("import", res.client_importer);
   ...
   get documents() {
      this.mergeDocument();
      //console.log("Base workspace documents", this.state.documents)
      return (
         <>
   ...
   componentDidMount() {
      ...
      //console.log("Base workspace did mount", this.props);
   }
   ...
   get pace() {
      ...
      return (
         <div className={`pace pace-${progress === 0 || progress > 100 ? 'active' : 'active'}`} ref={progress => this.progressBarr = progress} style={{ bottom: 0, display: "block" }}>
            <div className="pace-progress" data-progress-text={`${this.state.progress}%`} data-progress={this.state.progress} style={{ transform: `translate3d(${this.state.progress}%, 0px, 0px)`, top: this.headerHeight || 55 }}>
