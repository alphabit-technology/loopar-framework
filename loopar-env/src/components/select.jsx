...
export default class Select extends BaseInput {
   ...
   render() {
      ...
      return super.render(
         <div className={`select-2 select-${isOpen ? 'opened' : 'closed'}`}>
            <span className={`select2 select2-${data.size} ${baseClass} ${baseClass}--default ${baseClass}--${direction} ${focusClass} ${openedClass}`} style={{ width: '100%' }}>
               <span className="selection" ref={selection => this.selection = selection} onClick={e => {
                  e.stopPropagation();
                  e.preventDefault();
                  this.focus();
                  this.toggleClose();
                  ...
                  <span className={`select2-selection select2-selection--single${this.state.is_invalid ? ' is-invalid' : ''}`} role="combobox" aria-haspopup="true" aria-expanded="false" aria-disabled="false" aria-labelledby="select2-select2-data-remote-container">
                     <span className="select2-selection__rendered" role="textbox" aria-readonly="true">
                        {value.value || value.option ? [
                           ...
                           <div>{value.value || value.option}</div>
                        ] :
                           <span className="select2-selection__placeholder">{data.placeholder || data.label || 'Select an option'}</span>}
                     </span>
               ...
               <span className="select2-container select2-container--default select2-container--open select-loaded" style={{ position: 'absolute', left: 0, width: '100%' }} onMouseLeave={() => {
                  this.inside = false;
               }}>
                  <span ref={selector => this.selector = selector} className={`select2-dropdown select2-dropdown--${direction} ${valid ? '' : 'is-invalid'}`} style={{ ...{ width: '100%', position: 'inherit' }, ...(direction === "above" ? { top: '-280px' } : {}) }}>
                     <span className="select2-search select2-search--dropdown">
                        <input
                           className="select2-search__field"
                           ref={inputSearch => this.inputSearch = input}
                     ...
                     <span className="select2-results">
                        <ul className="select2-results__options" style={direction === "above" ? { height: "200px" } : {}}>
                           {rows.length > 0 ? rows.map(row => {
                              return (
                                 <li className="select2-results__option select2-results__option--higlighted" role="option" aria-selected="false" onClick={() => {
                                    this.setOptionSelect(row);
                                 }}>
                                    <div className="media">
                                       <div className="media-body">
                                          <h6 className="my-0">{this.optionValue(row).value || this.optionValue(row).option}</h6>
                           ...
                           }) : <li className="select2-results__option select2-results__message">No results found</li>}
                        </ul>
                     </span>
                  </span>
               </span>
