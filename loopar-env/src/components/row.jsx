...
export default class Row extends Component {
      ...
      return sizes.map((layout, layoutIndex) => {
         return (
            <div
               ...
               style={{ width: bottonSize + "%", borderRadius: 0, backgroundColor: 'transparent' }}
               onClick={(e) => {
                  e.stopPropagation();
                  e.preventDefault();
               ...
               {layout.map((size, index) => {
                  return <div className="progress-bar" style={{ width: size + "%", border: "unset", backgroundColor: `rgba(255,${Math.abs(150 - (index * 50))},0,${(index + 1) / layout.length})` }}></div>
               })}
            </div>
         )
      })
   ...
   render(content) {
      ...
      return super.render(
         <>
            {this.props.designer && (
               <div className="row-layout-selector" style={{ position: "absolute", bottom: 0, left: 0, width: "100%", display: "none", zIndex: 99 }}>
